...
const titleTest = "poolAPrioriPostManagementCreatePostDraft";
const pathScreenshotsTest = `./${nameScreenPath}/${titleTest}/`;
const dataPool = require("./poolSources/postManagementCreatePostDraftPool.json");
    ...
    for (let i = 0; i < dataPool.length; i++) {
      for (let item of dataPool) {
        ...
        await page.screenshot({
          path: pathScreenshotsTest + `./${version}_1good_login.png`,
        });

        //Abrir la URL a probar en la página y cargar el proyecto en una SPA
        await navigator.clickOnSidebar("posts");
        await page.screenshot({
          path: pathScreenshotsTest + `./${version}_2visit_posts.png`,
        });
        // Crear nueva post
        await navigator.clickOnNewEditor("post");
        await page.screenshot({
          path: pathScreenshotsTest + `./${version}_3new_post.png`,
        });

        if (typeof item[`item${i + 1}`] === "number") {
          console.log(
            "Error handled: Ghost does't not accept numbers as titles for pages and posts"
          );
          await browser.close();
          continue;
        }
        ...
        await page.screenshot({
          path: pathScreenshotsTest + `./${version}_4editing.png`,
        });
        // // salir del editor
        await navigator.saveAndFinishEditing("posts");
        await page.screenshot({
          path: pathScreenshotsTest + `./${version}_5returning_and_saving.png`,
        });

        await page.click("section .ember-view");
        await new Promise((r) => setTimeout(r, 1000));
        await page.screenshot({
          path: pathScreenshotsTest + `./${version}_6end_state.png`,
        });
