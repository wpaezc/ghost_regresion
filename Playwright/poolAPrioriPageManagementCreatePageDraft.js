...
(async () => {
  ...
  for (const browserType of ["chromium"]) {
    ...
    for (let item of dataPool) {
      ...
      await page.screenshot({
        path: pathScreenshotsTest + `./${version}_2visit_pages.png`,
      });
      // Crear nueva pagina
      await navigator.clickOnNewEditor("page");
      await page.screenshot({
        path: pathScreenshotsTest + `./${version}_3new_page.png`,
      });

      if (typeof(item[`item${i+1}`]) === 'number'){
        console.log("Error handled: Ghost does't not accept numbers as titles for pages and posts");
        await browser.close();
        continue;
      }
